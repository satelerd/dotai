...
repo owners de SmartUp (estas son las personas que deberian revisar los prs a dev o prod de cada repo):
## Repositorios con code owner asignado:

1. **shape-up** → SAT
2. **OpenCanvas** → Max
3. **ShapeUp-Autosolve** → Max
4. **smartvoc-backend** → Mati (VOCTOR)
5. **tools** → Alex
6. **ax** → Rafa
7. **admin** → Marcelo
8. **agente-0001** → Diego
9. **smartvoc-frontend** → Mati (VOCTOR)
10. **mintlify-docs** → Max
11. **SmartOrders** → Manu
12. **multi-channel** → Ludmi
13. **Auditorias-backend** → Mati (VOCTOR)

## Reglas al trabajar con servicios potencialmente productivos (clusters/DNS/cloud)
- Tratar todo como sensible: no ejecutar cambios sin aprobación explícita previa.
- Ojo con qué clúster estás operando: revisar siempre `kubectl config current-context` antes de actuar.
- Antes de cualquier cambio que pueda comprometer disponibilidad o información, definir plan de rollback.
